  ...
  files.forEach(function(file_name){
    if(file_name.endsWith('.cls')) {
      ...
      if(classModel) {
        ...
        // If we have specified specific groups to include, check that the
        // generated apex class matches one of the groups specified
        if(config.groups && config.groups.length > 0) {
          if(config.groups.contains(classModel.group)) {
